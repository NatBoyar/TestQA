...
@Описание: Экспортный сценарий, которые перезаполняет константу АдресSMTPСервера
@ПримерИспользования: И перезаполнение константы АдресSMTPСервера значением
